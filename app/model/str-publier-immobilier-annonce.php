		...
		if (!empty($descriptionAnnonce)) {
			if (!empty($numeroTelephone)) {
				...
				if (is_numeric($numeroTelephone)) {
					...
					$sql_inser = "INSERT INTO articles_annonces_immobilier (id_crypt, id_annonce, id_annonceur, categories, titre_annonce, adresse_annonceur, description, type_annonce, prix_apartement, unites, type_annonceur, numeroAppartement, nomDeRue, province, villeAppartement, typeDeMaison, salleDeBain, animaux, meubler, views, codePostal, adress_marchanise, numero_telephone, photos_array, date_annoncee, time_stamp)
