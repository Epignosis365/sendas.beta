		...
		if (!empty($descriptionAnnonce)) {
			if (!empty($numeroTelephone)) {
				...
				if (is_numeric($numeroTelephone)) {
					...
					$sql_inser = "INSERT INTO articles_annonces_autos (id_crypt, id_annonce, id_annonceur, categories, titre_annonce, adresse_annonceur, description, type_annonce, prix_marchandise, type_annonceur, marck, model, boiteDeVitesse, transmission, garniture, couleur, typeDeCarrosserie, typeDeCarburant, annee, nbreDePortes, conditions, nombreDePlaces, kilometre, nouveauPrix, views, codePostal, province, ville, adress_marchanise, numero_telephone, photos_array, date_annoncee, time_stamp)
