<?php
    ...
    $id = $_GET['?id'];
    //echo "<br>".base64_decode($id);
    include_once'db/ps-config.php';
	$data_SQL = 'SELECT * FROM articles_annonces_immobilier WHERE id_annonce = :id_annonce';
	...
	if ($count_data < 1) {
		...
		$animaux        = $result_data['animaux'];
		$meubler       = $result_data['meubler'];
		$unite_piece   = $result_data['unite_piece'];
		$typeDeMaison    = $result_data['typeDeMaison'];

		$numeroAppartement = $result_data['numeroAppartement'];
		$nomDeRue          = $result_data['nomDeRue'];
		$province          = $result_data['province'];
		$villeAppartement  = $result_data['villeAppartement'];

		$prix_marchandise  = $result_data['prix_apartement'];
		$unites  = $result_data['unites'];
		...
		$salleDeBains      = $result_data['salleDeBain'];
		...
		<div class="container-fliud" style="">
			<div class="row">
				...
				<div class="col-md-6" style="background-color: #fff;padding-top: 15px;border: 1px solid #e5e5e5;border-bottom: 1px solid #e5e5e5;">
					...
					<p class="vote"><i class="fa fa-map-marker"></i> <?= $numeroAppartement.' Rue '.$nomDeRue.', '.$villeAppartement.', '.$province.', '.$codePostal.' Canada' ?> </p>				
					<div class="rating">
						...
						<h4 class="price">Prix actuel : <span><?= number_format($prix_marchandise,2,","," "); ?>$</span> / <?= $unites ?></h4>
					</div>
					<div class="col-md-12" style="border-top: 1px dashed #ec2505;border-bottom: 1px dashed #ec2505; padding: 5px;display: inline-block;">
						<div class="col-md-6" style="padding: 0px;">
							<small class="infos-appart" style="color: #;">Meublé : <span style="color: #020202;"><?= $meubler ?></span></small>
						</div>
						<div class="col-md-6" style="padding: 0px;">
							<small class="infos-appart" style="color: #;">Animaux acceptés : <span style="color: #020202;"><?= $animaux ?></span></small>
						</div>
						<div class="col-md-6" style="padding: 0px;">
							<small class="infos-appart" style="color: #;">Type d'immobilier : <span style="color: #020202;"><?= $typeDeMaison ?></span></small>
						</div>
						<div class="col-md-6" style="padding: 0px;">
							<small class="infos-appart" style="color: #;">Salle de bain : <span style="color: #020202;"><?= $salleDeBains ?></span></small>
						</div>
					</div>
					<label style="font-size: 11px;font-weight: 300;color: #575858;">Contactez l'annonceur pour plus d'informations</label>
